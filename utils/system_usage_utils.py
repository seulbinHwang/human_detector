...
def monitor_gpu_usage(interval: float,
    ...
    Args:
        interval (float): Time interval in seconds for recording GPU usage.
        usage_data: Dictionary to store GPU usage and memory usage.
...
def monitor_system_usage(interval: float,
    ...
    """
    Monitors system CPU and RAM usage, including total and used memory,
        at regular intervals.

    Args:
        interval (float): Time interval in seconds for recording system usage.
        usage_data : Dictionary to store CPU usage, RAM usage,
            total memory, and used memory. Keys are "cpu_usage(%)",
            "ram_usage(%)", "total_memory(GB)", and "used_memory(GB)",
    ...
    # TODO: MPS 사용시, GPU 사용량 체크라는 로직 추가.
    ...
    while getattr(threading.currentThread(), "do_run", True):
        ...
        usage_data["ram_usage(%)"].append(vmem.percent)  # 사용 중인 메모리의 퍼센트
...
def _set_fps_text(usage_data: Dict[str, Dict[str, List[float]]],
                  system_info: str, interval: float, total_frames: int) -> None:
    for name, a_usage_data in usage_data.items():
        ...
        system_info += (f"\n[Data {name} Execution Time]: "
                        f"{execution_time:.2f} seconds.")
        system_info += f"\n[Data {name} FPS]: {fps:.2f} frames per second."
        system_info += (f"\n[Data {name} Execution Period]: "
                        f"{execution_period_per_frame:.3f} "
                        f"seconds per frame.\n")


def _match_data_length(
    usage_data_list: List[Dict[str, List[float]]],
    max_data_length: int,
):
    # 모든 데이터를 동일한 길이로 맞춤
...
def plot_multiple_usage_data(usage_data: Dict[str, Dict[str, List[float]]],
    ...
    """
    Plots multiple sets of CPU, RAM, and GPU (if available)
        usage data during model inference.
    Args:
        ...
        save (bool, optional): Whether to save the plot as an image.
            Default is False.
        save_path (str, optional): File path to save the plot (if save is True).
            Default is None.
    """
    ...
    _set_fps_text(usage_data, system_info, interval, total_frames)
    ...
    _match_data_length(usage_data_list, max_data_length)
...
def get_basic_system_info(device: torch.device) -> str:
    """ Get basic system information including CPU, GPU, and memory.

    Args:
        device: torch.device object.

    Returns:
        str: Basic system information.

    """
    cpu_cores_number = os.cpu_count()
    system_info = ""
    system_info += "======[Basic System Information]======"
    system_info += f"\n[Total CPU cores]: {cpu_cores_number}"
    ...
    if not device.type == "cpu":
        ...
        # elif torch.backends.mps.is_built():  # MPS
        #
        ...
        #     system_info += f"\n[Total GPU cores]: {total_gpu_cores}"
        #     system_info += (f"\n[MAC][Total GPU memory]"
        #                     f"(= mac shares memory with CPU): "
        #                     f"{total_memory_gb} GB ")
    system_info += "\n======================================"
    return system_info
...
def check_cpu_and_memory_usage(
    ...
    # TODO: 전체 시스템의 CPU, RAM 사용량을 측정하도록 구현되어 있음
    #  해당 이미지 처리 로직이 기여하는 CPU, RAM 사용량만 측정하도록 수정 필요
    if device.type == "cuda":
